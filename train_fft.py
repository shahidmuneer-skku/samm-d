...
from models.model_fftv2 import SAMMDModel
...
def main(args):
    ...
    train_loader = DataLoader(train_subset, batch_size=args.batch_size, drop_last=True, shuffle=True, num_workers=0, worker_init_fn=seed_worker)
    dev_loader = DataLoader(val_subset, batch_size=args.batch_size,  drop_last=True, shuffle=False, num_workers=0)

    # Create the model
    model = SAMMDModel(device, frontend=args.encoder)
    model = model.to(device)  # Move model to the primary device first
    model = nn.DataParallel(model, device_ids=[0,1,2,3])  # Wrap the model with DataParallel
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        writer.add_scalar("Train/AUC", auc_pr, epoch)
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(dev_loader)):
                ...
                #     break
                # audio, video, text, label, filenames = batch
            ...
            auc_pr = auc(recall, precision)
            ap_score = average_precision_score(all_labels, all_preds)
            writer.add_scalar("Val/AUC", auc_pr, epoch)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--log_dir", type=str, default="logs/new", help="The directory for the logs.")
