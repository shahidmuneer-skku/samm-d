...
def save_features(video, output_dir,row, model, processor):    
    ...
    video_type = "Fake" if row["label"]=="fake" else "Real"  
    
    labels_list = [f"Video is {video_type}"]
...
def extractFeatures(chunk, model_name, device):
    ...
    for index, row in chunk.iterrows():
        # video_path = list(chunk.keys())[index] 
        video_path = index
        video_path = os.path.join(base_dir,video_path)
        # print(row['label'])
        # exit()
        # fft_path = video_path.replace("_video_fft.mp4",".mp4")
...
def main():
    base_dir = ""
    csv_path = f'{base_dir}dataset.json'  # Change this to the actual path of your CSV file
