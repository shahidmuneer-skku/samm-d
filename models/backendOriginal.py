...
class AASIST(nn.Module):
    def __init__(self):
        ...
        self.multihead_attn = nn.MultiheadAttention(embed_dim=160, num_heads=8)
        filts = d_args["filts"]
        ...
        self.lstm = nn.LSTM(5 * gat_dims[1],5 * gat_dims[1],bidirectional=True)
        self.out_layer = nn.Linear(2 * 5 * gat_dims[1], 1)
    ...
    def forward(self, x, Freq_aug=False):
        ...
        # x = self.multihead_attn(last_hidden,last_hidden,last_hidden)
        last_hidden = self.drop_way(last_hidden)
        x,_ = self.lstm(last_hidden)
        # x = self.drop(x)
