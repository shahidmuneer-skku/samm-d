...
class SAMMDModel(nn.Module):
    def __init__(self, device, frontend=None):
        ...
        # Convolutional layers with pooling to downsample the video
        self.downsample_video = nn.Sequential(
            ...
            nn.Conv3d(in_channels=16, out_channels=32, kernel_size=(3, 3, 3), stride=(1, 2, 2), padding=1),
            nn.ReLU(),
            nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2)),
    ...
    def forward(self,audio, video, label, audio_fft, video_fft, text_embeddings, video_embeddings):
        ...
        audio_fft_embeddings = self.downsample_video(audio_fft)
        audio_fft_embeddings = torch.flatten(audio_fft_embeddings,start_dim = 2, end_dim=3)
        audio_fft_embeddings = self.transformer_encoder(audio_fft_embeddings)


        ori_video_embeddings = self.downsample_video(video)
        ori_video_embeddings = torch.flatten(ori_video_embeddings,start_dim = 2, end_dim=3)
        ori_video_embeddings = self.transformer_encoder(ori_video_embeddings)

        video_fft_embeddings = self.downsample_video(video_fft)
        video_fft_embeddings = torch.flatten(video_fft_embeddings,start_dim = 2, end_dim=3)
        video_fft_embeddings = self.transformer_encoder(video_fft_embeddings)

        ori_video_to_video_fft = self.transformer_fusion(ori_video_embeddings,video_fft_embeddings)
        video_embeddings_to_ori_video_to_video_fft = self.transformer_fusion(ori_video_to_video_fft,video_embeddings)
        audio_to_audio_fft = self.transformer_fusion(audio_embeddings,audio_fft_embeddings)  


        image_to_text = self.transformer_fusion(video_embeddings_to_ori_video_to_video_fft,text_embeddings)  
        audio_to_text = self.transformer_fusion(audio_to_audio_fft,text_embeddings)
        audio_to_video = self.transformer_fusion(image_to_text,audio_to_text)
        x = self.linear(audio_to_video[:, 0, :])
        # x = torch.sigmoid(x)  
