...
class SAMMDModel(nn.Module):
    ...
    def forward(self,audio, video, audio_fft, video_fft, text_embeddings, video_embeddings):
        ...
        # audio_to_text = torch.cat([audio_embeddings,text_embeddings], dim = 1)
        # audio_to_text = self.transformer_fusion(audio_to_text)
        ...
        transformer_out_video  = self.transformer_fusion_video(video_to_text, video_fft,video_fft)
        out = self.transformer_fusion(transformer_out_video,audio_to_text,audio_to_text)
        t = out.shape[0]
