...
# Define the Graph Attention Layer, HtrgGraphAttentionLayer, GraphPool, and CONV classes here if needed...
...
class AASIST(nn.Module):
    ...
    def forward(self, x, Freq_aug=False):
        ...
        x = x.transpose(1, 2)  # Shape: (batch_size, seq_length, features)
        
        x = x.transpose(0, 1)  # Transformer expects shape: (seq_length, batch_size, features)
        x = self.transformer_encoder(x)
        x = x.transpose(0, 1)  # Shape: (batch_size, seq_length, features)

        x = self.dropout(x)
        x = x.mean(dim=1)  # Pooling: Mean across the sequence length
