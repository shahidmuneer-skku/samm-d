...
class SAMMDModel(nn.Module):
    ...
    def process_video(self,video, labels_list):
        # Open video file

        text = self.tokenizer(labels_list, context_length=self.model.context_length).to(device) 
        frame_features = []

        with torch.no_grad(), torch.cuda.amp.autocast():
            image_features = model.encode_image(frame_tensor)
            frame_features.append(F.normalize(image_features, dim=-1))

        cap.release()

        frame_features = torch.cat(frame_features, dim=0)
        video_features = frame_features.mean(dim=0, keepdim=True)
    ...
    def forward(self,audio,videos,text):
        ...
        for i,video in enumerate(videos):
            ...
            print(video.shape)
            # inputs = self.clip_processor(text=[text[i]], images=video, return_tensors="pt", padding=True, do_rescale=False)
            # inputs = {k: v.to(self.clip_model.device) for k, v in inputs.items()}
            # outputs = self.clip_model(**inputs)
            # print(outputs.image_embeds.shape)
            # image_embedding=self.reducer(outputs.image_embeds)
            # text_embedding=self.reducer(outputs.text_embeds)
            text_features, video_features  = self.process_video(video, text)
            image_embeddings.append(video_features)
            text_embeddings.append(text_features) 
        image_embeddings=torch.stack(image_embeddings)
        text_embeddings=torch.stack(text_embeddings)
        # audio_embeddings= audio_embeddings.permute(0,2,1)
        # print(audio_embeddings.shape)
        # print(image_embeddings.shape,text_embeddings.shape,audio_embeddings.shape)
        ## combined_embeddings = torch.cat((image_embeddings,audio_embeddings, text_embeddings), dim=1)
        # print(combined_embeddings.shape)
        # lstm_out, (hidden, cell) = self.lstm(combined_embeddings)
        # lstm_out = lstm_out[:, -1, :]  # Get the last time step output
        # print(lstm_out.shape)
        ## out = self.transformer_encoder_out(combined_embeddings)
        # print(out.shape)
        # print(out[:, 0, :].shape)

        image_to_text = self.transformer_fusion(image_embeddings,text_embeddings) # (32, 82,512) + (32,512,512)
        audio_to_text = self.transformer_fusion(audio_embeddings,text_embeddings)
